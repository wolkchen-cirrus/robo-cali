__doc__ = """
This python module is responsible for handling the JSON
config file, including its initial creation.
"""
...
config_path = os.path.join(os.path.split(
    os.path.abspath(__file__))[0], config_fn)
...
class _ConfVal:
    def __init__(self, cd):
        ...
        self.unit = None
        self.desc = None
        [setattr(self, k, v) for k, v in cd.items()
    ...
    while True:
        ...
        try:
            ...
            raise FileExistsError(
                "Config with name %s already exists" % cc.name
            )
...
def _getitem(name, fcd):
    item_list = [(i, x) for i, x in enumerate(fcd)
                 if name == x['name']]
    if len(item_list) != 1:
        if len(item_list) > 1:
            raise AttributeError(
                'More than one setting of name %s exists' % name
            )
        else:
